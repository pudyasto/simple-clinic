...
class RptPendapatanController extends Controller
{
    //
    ...
    public function previewData(Request $request){
        ...
        return view('laporan.pendapatan.pdf', [
            'data' => $data,
        ]);
    }
    ...
    public function printData(Request $request){
        ...
        $mpdf->WriteHTML(view('laporan.pendapatan.pdf', [
            'data' => $data,
        ]));
    ...
    private function getData($request){
        $registrasi = Kasir::with(['poli', 'pasien', 'pegawai'])
            ->whereRaw(" date_format(tgl_bayar, '%Y-%m-%d') BETWEEN '". dateconvert($request->tgl_mulai) ."' AND '". dateconvert($request->tgl_selesai) ."' ")
            ->get();
        return $registrasi;
    }
}
